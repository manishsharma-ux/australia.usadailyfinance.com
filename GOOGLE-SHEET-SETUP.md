...
When the phone number is entered and the user clicks Next, the form texts a
6-digit code; once that code checks out, every answer is written to your Google
Sheet and the browser lands on `thanks.html`.

The SMS half of that is **[TWILIO-OTP-SETUP.md](TWILIO-OTP-SETUP.md)** — same
Apps Script deployment, three extra Script Properties. Set
`SKIP_OTP_VERIFICATION = true` in `index.html` to go back to writing the row
straight from the phone step with no code at all.
...
var SKIP_OTP_VERIFICATION   = false;   // true writes the row with no SMS code
var AU_PHONE_ALLOW_LANDLINE = false;   // a landline cannot receive the code
...
**Zapier.** `process.php` is gone from this project entirely, so the Zapier
webhooks and notification email it used to fire no longer do. Set `ALSO_SEND_TO_ZAPIER = true` to POST
...
- **The endpoint is no longer open for leads with a phone number.** Since SMS
  verification went in, `handleLead` rejects any lead carrying a number without
  the token minted by a successful code check, so a `curl` at the `/exec` URL
  cannot fill the sheet. The two paths still open by design — an unconfigured
  Twilio, and the renter referral, which has no number — are set out at
  `checkOtpGate` in `Code.gs`.
- **Phone numbers are proven, names and addresses are not.** The code check
  says the handset answered; nothing checks that the name or address beside it
  is real. It is the field worth proving, but it is one field.
...
The line under the question now reads *"We'll text you a 6-digit code to
confirm this number. An accredited installer will call you on it."* — true
again since SMS verification came back, and it says what happens next rather
than only what is required.
...
**Worth weighing.** This table predates SMS verification, and with
`SKIP_OTP_VERIFICATION = false` the second column no longer decides much: a
number that passes every format rule still has to receive a text before the row
is written, so `12345` gets as far as the code step and no further. The format
settings now shape the error the visitor sees rather than what reaches the
sheet. They matter again the moment verification is switched off.

---

## 10. "There was some problem" on the phone step

That text no longer exists anywhere in this project. It came from the old
`sendCode()`, which POSTed to a `process.php` that this project does not
contain:
...
`sendCode()` now posts to the Apps Script endpoint and reports what that
endpoint actually said. So seeing that sentence at all proves the server is
serving the **old** `custom.js` — it is the single clearest sign of a stale
upload, and `?debug=1` says the same thing in more detail.
