...
(function () {
    ...
    var EXPECTED_BUILD = 'otp-crm-2026-09-22';
    ...
    function check() {
        ...
        var hasOtp       = typeof window.sendCode === 'function' && typeof window.verifyCode === 'function';
        ...
        // With SMS verification on, the lead is only written after Twilio
        // approves the code — so an endpoint that cannot send one stops every
        // lead, not just the verification. It is a failure, not a note.
        var otpOn   = typeof SKIP_OTP_VERIFICATION !== 'undefined' && !SKIP_OTP_VERIFICATION;
        var otpOk   = !otpOn || hasOtp;
        var otpNote = !otpOn ? 'off — SKIP_OTP_VERIFICATION is true, leads go straight to the sheet'
                    : hasOtp ? 'on — Twilio Verify, via the Apps Script endpoint'
                             : 'on, but sendCode/verifyCode are undefined — custom.js did not load';
        ...
        var broken = !hasJq || !hasCustom || !bound || !customOk || !mapsOk || !otpOk;
        ...
        render([
            ...
            ['SMS verification',         otpOk,     otpNote]
        ], broken);
    }
    ...
    function render(rows, broken) {
        ...
        if (buildMismatch) {
            ...
            if (typeof window.sendCode === 'function' && typeof window.srSubmitLead !== 'function') {
                html += '<div style="margin-top:6px;color:#ffd68a;">The custom.js being served is the old one: ' +
                        'it posts to process.php, which this project does not contain. That is where ' +
                        '"There was some problem" comes from.</div>';
            }
    ...
    function describePong(res) {
        ...
        // Only a problem when the form actually asks for codes. A deployment
        // with no Twilio credentials is the correct state while OTP is off.
        var wantsOtp = typeof SKIP_OTP_VERIFICATION !== 'undefined' && !SKIP_OTP_VERIFICATION;
        if (wantsOtp && res.otp !== 'configured') {
            return { ok: false, note: where + ' — SMS verification is ON but the deployment ' +
                'cannot send codes: ' + (res.otp_error || 'no Twilio credentials') +
                '. Apps Script -> Project Settings -> Script Properties.' };
        }

        // CRM forwarding fails silently by design — the visitor still sees a
        // thank-you page and the row still reaches the sheet. The only place a
        // missing secret or a deleted trigger shows up is here.
        var crm = res.crm;
        if (crm && crm.error) {
            return { ok: false, note: where + ' — CRM outbox unreadable: ' + crm.error };
        }
        if (crm && !crm.configured) {
            return { ok: false, note: where + ' — leads reach the sheet but NOT the CRM: ' +
                'CRM_URL / CRM_SECRET are not set. Apps Script -> Project Settings -> ' +
                'Script Properties, then run crmSetup().' };
        }
        if (crm && !crm.trigger) {
            return { ok: false, note: where + ' — CRM queue is never drained: the every-minute ' +
                'crmFlush trigger is missing. Run crmSetup() from the Apps Script editor.' };
        }

        var queued = crm && crm.outbox
            ? ', CRM outbox: ' + crm.outbox.sent + ' sent / ' +
              (crm.outbox.pending + crm.outbox.retrying) + ' waiting'
            : '';
        return { ok: true, note: where + ', build ' + build + queued +
